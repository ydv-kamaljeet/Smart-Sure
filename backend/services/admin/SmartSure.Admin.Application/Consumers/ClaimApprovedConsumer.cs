...
public class ClaimApprovedConsumer : IConsumer<ClaimApprovedEvent>
{
    ...
    public async Task Consume(ConsumeContext<ClaimApprovedEvent> context)
    {
        ...
        _logger.LogInformation("Received ClaimApprovedEvent for ClaimId {ClaimId}", message.ClaimId);
        ...
        if (localClaim != null)
        {
            localClaim.Status = "Approved";
            localClaim.ClaimAmount = message.ApprovedAmount;
            localClaim.UpdatedAt = DateTime.UtcNow;
            await _claimRepo.UpdateAsync(localClaim);
            await _unitOfWork.SaveChangesAsync();
            _logger.LogInformation("Updated ClaimId {ClaimId} to Approved.", message.ClaimId);
        }
        ...
        if (user != null)
        {
            ...
            var body = $@"
                <div style='font-family:sans-serif;max-width:600px;margin:auto;'>
                  <h2 style='color:#198754;'>Claim Approved</h2>
                  <p>Dear <strong>{user.FullName}</strong>,</p>
                  <p>Great news! Your claim <strong>{claimNumber}</strong> has been <strong>approved</strong>.</p>
                  <p><strong>Approved Amount:</strong> &#8377;{message.ApprovedAmount:N0}</p>
                  <p><strong>Remarks:</strong> {message.Remarks}</p>
                  <p>The settlement will be processed shortly. Thank you for choosing SmartSure.</p>
                  <br/>
                  <p style='color:#6c757d;font-size:0.85rem;'>SmartSure Insurance</p>
                </div>";

            await _emailService.SendEmailAsync(user.Email, $"Claim {claimNumber} Approved", body);
        }
    }
}
